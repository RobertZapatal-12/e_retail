...
def run_products_seed(products_repo):
    ...
    for i in range(1, NUM_BRANDS + 1):
        ...
        name = unique_value(name, used_brands)

        products_repo.insert_brands(i, name)
    ...
    for i in range(1, NUM_CATEGORIES + 1):
        ...
        slug = create_slug(name, used_category_slugs)

        products_repo.insert_category(i, name, slug, None, fake.text(max_nb_chars=100))
    ...
    for product_id in products_ids:
        ...
        products_repo.insert_productcategories(product_id, category_id)
    ...
    for i, attribute in enumerate(attributes, start=1):
        products_repo.insert_product_attribute_keys(i, attribute[0], attribute[1])
    ...
    variant_names = ["Base", "Pro", "Gaming", "Ultra", "Plus"]
    ...
    colors = ["Negro", "Blanco", "Rojo", "Azul", "Gris"]
    ...
    for i, variant_id in enumerate(variants_ids, start=1):
        ...
        products_repo.insert_variant_attribute_values(
            i, variant_id, attribute_id, random.choice(colors)
        )
    ...
    for i in range(1, NUM_SUPPLIERS + 1):
        ...
        products_repo.insert_suppliers(
            i, fake.company(), fake.name(), email, fake.phone_number(), fake.address()
        )
    ...
    for product_id in products_ids:
        ...
        products_repo.insert_product_suppliers(
            product_id, supplier_id, f"SUP-{product_id}", random.randint(1, 30)
        )
    ...
    for i, inventory_id in enumerate(inventory_ids, start=1):
        products_repo.insert_inventory_transactions(
            ...
            random.choice(["Compra proveedor", "Venta cliente"]),
