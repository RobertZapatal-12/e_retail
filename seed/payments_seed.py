...
def run_payments_seed(payments_repo):
    ...
    for code, name in currencies:
        payments_repo.insert_currencies(code, name)
    ...
    for i, method in enumerate(payment_methods, start=1):
        payments_repo.insert_payment_methods(i, method[0], method[1])
    ...
    for i, status in enumerate(payment_statuses, start=1):
        payments_repo.insert_payment_statuses(i, status[0], status[1])
...
def run_payments_data_seed(payments_repo):
    ...
    for i in range(1, NUM_PAYMENTS + 1):
        payments_repo.insert_payments(
            ...
            random.randint(1, 5),
            random.randint(1, 4),
            random.choice(["Stripe", "PayPal", "Visa", "Mastercard"]),
