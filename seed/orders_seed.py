...
def run_orders_seed(
    ...
    for i, status in enumerate(order_statuses, start=1):
        orders_repo.insert_order_statuses(i, status[0], status[1])

    # =========================
    # CARRIERS
    # =========================

    carriers = ["DHL", "FedEx", "UPS", "Caribe Express", "Metro Pac"]

    for i, carrier in enumerate(carriers, start=1):
        orders_repo.insert_carriers(i, carrier, fake.phone_number())
    ...
    for i, status in enumerate(shipment_statuses, start=1):
        orders_repo.insert_shipment_statuses(i, status[0], status[1])
    ...
    for i, promo in enumerate(promotion_types, start=1):
        orders_repo.insert_promotion_types(i, promo[0], promo[1])
